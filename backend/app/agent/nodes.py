...
1. FOLLOW-UP DETECTION
   ...
   - "yes", "ok", "sure", "2", "30 minutes", an email address, a 6-digit code → meeting_scheduler 0.99.
...
4. GREETINGS, CHITCHAT & CLOSINGS
   If the user says things like "hi", "hello", "hey", "good morning", "how are you", "bye", "goodbye", "thank you", "thanks", "see you later",
   classify as general_qa, sub_intent: general with confidence 1.0.

5. AMBIGUOUS / VERY SHORT MESSAGES (without clear history context)
...
══ JOB SEARCH SUB-INTENT GUIDE ══
  ...
  job_detail          → "tell me more about the Senior Data Engineer role"
"""
...
def intent_detection_node(state: AgentState) -> dict:
    ...
    job_search_threshold = 0.70
    all_above = all(
        i.confidence >= (job_search_threshold if i.intent == IntentType.JOB_SEARCH else threshold)
        for i in intents
    )
...
async def _run_job_search(extracted_params: dict, query: str, session_id: str, cost_log: LLMCostLog) -> dict:
    ...
    search_result = await search_jobs(role=role, location=location, status=status)
    jobs_text = format_jobs_for_llm(search_result)

    # Use LLM to compose a natural answer from job data
    model = settings.OPENAI_MODEL
    system_msg = """You are MHK Tech's friendly AI assistant answering job-related questions.
