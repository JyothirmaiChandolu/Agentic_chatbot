...
async def handle_message(
    ...
    # -------------------------------------------------------------------------
    # COMPLETED — restart session if they write again
    # -------------------------------------------------------------------------
        ...
        new_session = MeetingSession(
            ...
            created_at=str(int(datetime.now(timezone.utc).timestamp() * 1000)),
            sub_intent="normal_meet",
            calendly_url=settings.CALENDLY_30MIN_URL,
        )
        save_session(new_session)
        return (
            "To schedule another meeting, please provide your **business email address**."
        ), new_session
